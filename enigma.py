...
Original file is located at
    https://colab.research.google.com/drive/18fFkKCquw2K1wm8i6Kt2nFYJHOt2VG5D
"""
...
def encode(plaintext, inputRotors, reflector):
  global ringSettings,ringPositions,plugboard
  ...
  rotorA = rotorDict[rotors[inputRotors[0]]]
  rotorB = rotorDict[rotors[inputRotors[1]]]
  rotorC = rotorDict[rotors[inputRotors[2]]]
  rotorANotch = rotorNotchDict[rotors[inputRotors[0]]]
  rotorBNotch = rotorNotchDict[rotors[inputRotors[1]]]
  rotorCNotch = rotorNotchDict[rotors[inputRotors[2]]]
...
if eord == 1:
  message = input('Type your plaintext: ')
elif eord == 2:
  message = input('Type your cyphertext: ')
else:
...
chooseRotors = input('Type your rotors as a 3 digit number: ')

choosenRotorsList = []

choosenRotorsList = list(chooseRotors)

print(choosenRotorsList)

if len(choosenRotorsList) != 3:
  raise Exception('I only asked for three numbers')

if '1' in choosenRotorsList:
  rotorsList.append(0)
if '2' in choosenRotorsList:
  rotorsList.append(1)
if '3' in choosenRotorsList:
  rotorsList.append(2)
if '4' in choosenRotorsList:
  rotorsList.append(3)
if '5' in choosenRotorsList:
  rotorsList.append(4)

if len(choosenRotorsList) != 3:
  raise Exception('I only asked for three numbers')

chosenReflector = input("Choose Reflector B or C: ")

if chosenReflector == 'B' or chosenReflector == 'b':
  encoded_message = encode(message, rotorsList, 0)
elif chosenReflector == 'C' or chosenReflector == 'c':
  encoded_message = encode(message, rotorsList, 1)
else:
  raise Exception('Type only B or C')
